...
// error: Argument of type 'null' is not assignable to parameter of type
// 'Element'. getId(null);
function getID(x: Element|null) { return x!.id; }
// getID(null);
// runtime error
...
function setNextValue(node: ListNode, value: number) {
  addNext(node);
  // Even though we know `node.next` is defined because we just called
  // `addNext`, TypeScript isn't able to infer this in the line of code below:
